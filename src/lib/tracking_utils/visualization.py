...
def plot_tracks(image,
    ...
    text_scale = max(0.8, image.shape[1] / 1600.)  # 1600.
    # text_thickness = 1 if text_scale > 1.1 else 1
    ...
    for cls_id in range(num_classes):
        ...
        for i, tlwh_i in enumerate(cls_tlwhs):
            ...
            # 文字背景框（先画背景，再写字）
            text_size, _ = cv2.getTextSize(id_text, cv2.FONT_HERSHEY_PLAIN, text_scale, text_thickness)
            text_w, text_h = text_size
            cv2.rectangle(img,
                          (int(x1), int(y1)),
                          (int(x1) + text_w, int(y1) + text_h + 4),
                          color=color,  # 背景色与边框相同
                          thickness=-1)

            # 绘制文字
            cv2.putText(img,
                        id_text,
                        (int(x1), int(y1) + text_h),
                        cv2.FONT_HERSHEY_PLAIN,  # 更接近 Times New Roman 的字体
                        text_scale,
                        (255, 255, 255),  # 白色文字
                        thickness=text_thickness,
